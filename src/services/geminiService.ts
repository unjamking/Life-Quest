...
// FIX: Adhered to Gemini API guidelines by using process.env.API_KEY directly for initialization. This resolves errors related to Vite-specific environment variables.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

export const getAIChatResponse = async (history: ChatMessage[], newMessage: string): Promise<string> => {
    // The `try...catch` block will handle runtime API errors.
    const fullHistory = [
        ...history,
